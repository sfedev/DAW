...
public class CsvProcessorGranjeros {

    public static void insertIntoGranjeros(Connection conn, File directoryPath) {
    ...
    private static void updateRecordInDatabase(Connection conn, String[] newValues) {
        String updateQuery = "UPDATE Granjeros SET nombre=?, descripcion=?, dinero=?, puntos=?, nivel=? WHERE id=?";

        try (PreparedStatement preparedStatement = conn.prepareStatement(updateQuery)) {
            // Configura los parámetros del PreparedStatement con los nuevos valores
            preparedStatement.setString(1, newValues[1]); // nombre
            preparedStatement.setString(2, newValues[2]); // descripcion
            preparedStatement.setFloat(3, Float.parseFloat(newValues[3])); // dinero
            preparedStatement.setInt(4, Integer.parseInt(newValues[4])); // puntos
            preparedStatement.setInt(5, Integer.parseInt(newValues[5])); // nivel
            preparedStatement.setInt(6, Integer.parseInt(newValues[0])); // id
